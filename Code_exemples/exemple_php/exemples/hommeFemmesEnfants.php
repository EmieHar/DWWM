...
<main>
<!--afficher chaque catégorie lors click dans hommes,femmes ou enfants-->
<?php
    // print_r($cata['hommes']);
   ...
   if(isset($_GET['categ'])){
    ...
    $produits = $cata[$categorie];?>

 
    <h1><?= ucfirst($categorie)?></h1> 
    <hr>
    
   <?php foreach($produits as $cle=>$valeur){?>
         
            <h2><?=$valeur["nom_produit"]?></h2>
            <img src="<?="https://".$valeur["photo"]?>">
            <p><?=$valeur["prix"]?> Euros</p>
            <hr>
       <?php 
   } 
